 ...
 */
function chart(elem_id, dates, counts, baseurl, prepend_elem_id) {
    function merge(dates, counts) {
        result = []
        for(i = 0; i < dates.length; i++) {
            result.push({
    ...
    var svg;
    // you can append chart (default behavior)
    if (typeof prepend_elem_id === "undefined" || prepend_elem_id === null) {
		svg = d3.select(elem_id).append("svg");
	}
	// or prepend (shown as first child of selected element)
	else {
		svg = d3.select(elem_id).insert("svg", prepend_elem_id);
	}
	svg.attr("id", "chart-data")
        .attr("width", width + margin.left + margin.right)
        .attr("height", height + margin.top + margin.bottom)
		.append("g")
        .attr("transform", "translate(" + margin.left + "," + margin.top + ")");
